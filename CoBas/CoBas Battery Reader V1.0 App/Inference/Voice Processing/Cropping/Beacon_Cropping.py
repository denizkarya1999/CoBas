...
def save_segments(audio: np.ndarray, sample_rate: int, output_dir: Path, base_name: str) -> int:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Crop protocol audio using beacon markers.")
    parser.add_argument("input")
    parser.add_argument("--output-dir", type=Path, required=True)
    args = parser.parse_args()

    crop_by_beacon(Path(args.input), args.output_dir)
