...
SEGMENTATION_FOLDER = "Beacon_Segmentation"
...
def segment_video_by_beacon(
    input_video: Path = Path(INPUT_VIDEO),
    output_folder: Path = Path(SEGMENTATION_FOLDER),
) -> bool:
...
def main() -> None:
    ...
    parser.add_argument("--output-folder", type=Path, default=Path(SEGMENTATION_FOLDER))
